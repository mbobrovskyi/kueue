...
// warningsForWorkload returns non-blocking warnings for values that are
// discouraged but still accepted for API backward compatibility. A negative
// subGroupCount is meaningless and is treated as unset by the scheduler; it is
// slated to become a hard validation error in a future release (see #13061).
