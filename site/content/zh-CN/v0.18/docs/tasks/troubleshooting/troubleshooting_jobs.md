...
的故障，
但大多数想法可以推广到其他[支持的作业类型](/v0.18/docs/tasks/run)。

请参阅 [Kueue 概览](/v0.18/docs/overview/#high-level-kueue-operation)
以可视化协作运行 Job 的组件。
...
对于每个 Job，Kueue 创建一个 [Workload](/v0.18/docs/concepts/workload) 对象来保存
...
一旦你[识别了你的 Job 对应的 Workload](/v0.18/docs/tasks/troubleshooting/troubleshooting_jobs/#identifying-the-workload-for-your-job)，
...
有关更多信息，请参阅[资源组](https://kueue.sigs.k8s.io/v0.18/docs/concepts/cluster_queue/#resource-groups)。

### 等待准入检查 {#pending-admission-check}

当 ClusterQueue 配置了[准入检查](/v0.18/docs/concepts/admission_check)时，例如
[ProvisioningRequest](/v0.18/docs/admission-check-controllers/provisioning) 或 [MultiKueue](/v0.18/docs/concepts/multikueue)，
...
当使用具有 `StrictFIFO`
[`queueingStrategy`](/v0.18/docs/concepts/cluster_queue/#queueing-strategy) 的
[ClusterQueue](/v0.18/docs/concepts/cluster_queue) 时，Kueue 只尝试准入每个 ClusterQueue 的头部。
...
请参阅[队列故障排除](/v0.18/docs/tasks/troubleshooting/troubleshooting_queues)了解为什么
ClusterQueue 或 LocalQueue 不活跃。

## 我的 Job 是否被抢占？ {#is-my-job-preempted}

如果你的 Job 没有运行，并且你的 ClusterQueue 启用了[抢占](/v0.18/docs/concepts/cluster_queue/#preemption)，
